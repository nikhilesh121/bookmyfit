    ...
    <Shell title="Check-ins">
      ...
      <div className="grid grid-cols-3 gap-4 mb-8">
        {[
          ...
          { label: 'Valid Today', value: checkins.filter(c => c.status === 'valid').length, icon: CheckCircle, color: 'var(--accent)' },
          { label: 'Flagged', value: checkins.filter(c => c.status === 'flagged').length, icon: Clock, color: '#FF3C3C' },
        ].map((s) => {
          const Icon = s.icon;
          return (
            <div key={s.label} className="card stat-glow p-5">
      ...
      <div className="glass overflow-hidden">
        <table className="glass-table">
          <thead>
            <tr>
              <th>User</th><th>Time</th><th>Method</th><th>Status</th>
            </tr>
          </thead>
          <tbody>
            {loading
              ...
              : checkins.length === 0
                ? <tr><td colSpan={4} style={{ textAlign: 'center', color: 'var(--t2)', padding: '40px 0' }}>No check-ins found</td></tr>
                : checkins.map((c) => (
                  <tr key={c.id}>
