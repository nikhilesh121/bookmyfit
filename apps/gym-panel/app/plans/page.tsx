...
function TypeBadge({ type }: { type: string }) {
  const colors: Record<string, React.CSSProperties> = {
    ...
    Couple: { background: 'rgba(255,100,180,0.15)', color: '#FF64B4' },
    Family: { background: 'rgba(61,255,84,0.12)', color: 'var(--accent)' },
    Elite: { background: 'rgba(180,120,255,0.18)', color: '#B478FF' },
    Corporate: { background: 'rgba(255,180,0,0.15)', color: '#FFB400' },
  };
  const s = colors[type] || { background: 'rgba(255,255,255,0.08)', color: 'var(--t)' };
  return (
...
export default function PlansPage() {
  ...
  const stats = [
    { label: 'Total Plans', value: loading ? '-' : totalPlans, icon: CreditCard },
    { label: 'Active Plans', value: loading ? '-' : activePlans, icon: CreditCard },
    { label: 'Total Subscribers', value: loading ? '-' : totalSubscribers, icon: Users },
  ];

  return (
    <Shell title="Plans &amp; Pricing">
      {/* Stats Row */}
      <div className="grid grid-cols-3 gap-4 mb-6">
        {stats.map((stat) => {
          const Icon = stat.icon;
          return (
            <div key={stat.label} className="card stat-glow p-5 flex items-center gap-4">
              <div
                style={{
                  background: 'rgba(61,255,84,0.1)',
                  borderRadius: 10,
                  padding: 10,
                  display: 'flex',
                  alignItems: 'center',
              ...
              <div>
                <div className="text-xs font-semibold kicker mb-1">{stat.label}</div>
                {loading ? (
                  <div
                    style={{
                      height: 24,
      ...
      <div className="glass overflow-hidden mb-6">
        <div className="p-5 pb-0">
          <h2 className="serif text-lg">All Plans</h2>
        </div>
              ...
              {loading
                ...
                : plans.map((plan) => (
                    <tr key={plan.id}>
                      ...
                      <td style={{ color: 'var(--t)' }}>
                        &#8377;{plan.price.toLocaleString('en-IN')}
                      </td>
                      <td style={{ color: 'var(--t2)' }}>{plan.duration}</td>
                      <td style={{ color: 'var(--t)' }}>{plan.activeSubscribers}</td>
                      <td>
                        <button
                        ...
                        >
                          {plan.active ? (
                            ...
                            <>
                              <ToggleLeft size={20} color="var(--t2)" />
                              <span style={{ color: 'var(--t2)', fontSize: 13, fontWeight: 600 }}>
                                Inactive
                              </span>
                            </>
                          )}
                      ...
                      <td>
                        <button className="btn btn-ghost text-xs">View Details</button>
                      </td>
                    </tr>
                  ))}
            </tbody>
      ...
      {/* Info Footer */}
      <div className="glass card p-4 flex items-start gap-3" style={{ borderColor: 'rgba(255,255,255,0.06)' }}>
        <AlertTriangle size={16} color="var(--t3)" style={{ marginTop: 2, flexShrink: 0 }} />
        <p style={{ color: 'var(--t2)', fontSize: 13, lineHeight: 1.6 }}>
          Plan availability controls which plans members can subscribe to at your gym.
        </p>
      </div>
    </Shell>
  );
